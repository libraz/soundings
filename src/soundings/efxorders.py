...
LIMITS = (
    ...
    "all -- what stands halfway to the next order, read through the same filter, in this "
    ...
    "**Halfway to the next order is not empty by construction, and on some stimuli it is "
    "the least empty place in the gap.** A voice whose partials sit at half multiples of "
    "its own fundamental puts a line exactly there, and one of this unit's own voices "
    "does: sixty-six to eighty-three decibels under its first partial, the same series at "
    "every note it was asked at, standing about eleven decibels over the median of the "
    "gap it sits in and tens of decibels over the emptiest points either side of it. So "
    "this figure bounds what is at that frequency, which is what it is for when an order "
    "is being called an order, and it is a floor only where there is nothing there. Which "
    "of the two it is for a given stimulus is not something this reading can tell, and "
    "the stimulus is stated beside it. "
    ...
    "**`Fed one steady tone` is a condition and not a preamble, and a stimulus carrying "
    "its own orders does not meet it.** Where it is not met these angles are not a "
    "reading about the stage at all, and neither is how they move across a sweep. Order "
    "`k` then collects products of the stimulus's own components alongside whatever the "
    "stage's own `k`-th term puts there -- landing on the same frequency, in the same "
    "bin, separable by no floor in this record or any take beside it, because the take "
    "routed past the stage never made them. The two grow at different rates as the level "
    "rises, so their sum turns as a sweep proceeds with nothing inside the stage turning. "
    "A stimulus is one steady tone only if it carries no orders of its own; how much it "
    "carries is stated with the stimulus, and where a record's control takes show any, "
    "these angles belong to the voice. "
