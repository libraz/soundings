...
def test_only_the_second_setting_is_preceded_by_a_reset_of_its_own():
    """The run already resets before the stimulus, so resetting before the first
    setting again would buy the state it is in. The second is where it matters:
    without it that setting inherits everything the first one's takes left, which
    is what makes a parameter deciding whether a message is received invisible."""
    from soundings.cli import sound

    assert sound.settings_reset_before((1, 0), True) == [0]
    assert sound.settings_reset_before((0, 127), True) == [127]


def test_a_run_that_did_not_ask_resets_between_no_settings():
    """The guard has to have an outside. Resetting always would ask every
    parameter from the power-on state rather than from the setting before it,
    which is a different question from the one every existing record answers."""
    from soundings.cli import sound
